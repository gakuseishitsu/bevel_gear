...
def run(context):
    ...
    try:
        ...
        neu = np.linspace(neu_range_neg-0.3, neu_range_pos+0.3, resolution_neu) # deg

        '''
        X_neu_theta: Coordinate values of the imaginary crown gear tooth surface point cloud
        '''
        theta = np.linspace(-1.0 * (0.8 * module)/(np.cos(np.radians(alpha))), (1.25 * module)/(np.cos(np.radians(alpha))), resolution_theta) # mm
        ...
        for index_theta, theta_n in enumerate(theta):
            for index_neu, neu_n in enumerate(neu):
                for index_psi, psi_n in enumerate(psi):
                    ...
                    relative_ang_velocity = np.array([0, 1.0 / np.tan(np.radians(delta_p0)), 0])
        ...
        for index_psi, psi_n in enumerate(psi):
            for index_neu, neu_n in enumerate(neu):
                if Theta_neu_psi[index_neu][index_psi] > 0:
                    z_axis_rotation = 1.0 * psi_n / np.sin(np.radians(delta_p0))
